...
groups are **mutually exclusive**. Supported languages: Java, PHP. Unsupported languages are rejected explicitly; use
`file` + `line` + `column` for other languages.
...
quick-fix intentions may be less complete unless the file is already open in an editor.
